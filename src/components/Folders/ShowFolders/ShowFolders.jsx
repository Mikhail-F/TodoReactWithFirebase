...
let ShowFolders = (props) => {
    ...
    useEffect(() =>{
        ...
        return (
            <>
                <div>
                    <button onClick={activeEditMode} className={c.addFolder}>+ Добавть папку</button>
                </div>
                <div className={c.innerShowFolder}>
                    {editMode && <div className={c.showAddFolder}>
                        <InputFormContainer onSubmit={getNewFolder} {...props} getColors={getColors} activeEditMode={activeEditMode} selectColorId={selectColorId}/>
                    </div>}
                </div>
            </>
        )
